...
Route::middleware('auth:sanctum')->group(function () {
    ...
    Route::prefix('produk')->middleware('role:admin,karyawan')->group(function () {
    ...
    Route::prefix('pesanan')->group(function () {
        ...
        // Staff/Admin only
        Route::put('/{pesanan}/status', [PesananController::class, 'update'])->middleware('role:admin,karyawan');
    });
    ...
    Route::prefix('booking')->group(function () {
        ...
        // Staff/Admin only
        Route::post('/{booking}/verify', [BookingController::class, 'verify'])->middleware('role:admin,karyawan');
    });
    ...
    Route::post('/pesanan/{pesanan}/pay', [TransaksiController::class, 'payOrder']);
    Route::post('/booking/{booking}/pay', [TransaksiController::class, 'payBooking']);
    ...
    Route::prefix('ulasan')->group(function () {
        ...
        // Admin only
        Route::post('/{ulasan}/toggle-visibility', [UlasanController::class, 'toggleVisibility'])->middleware('role:admin');
    });
    ...
    Route::prefix('chatbot')->group(function () {
        ...
        Route::post('/conversations', [ChatbotLogController::class, 'startConversation']);
        Route::post('/conversations/{chatbotLog}/message', [ChatbotLogController::class, 'sendMessage']);
        Route::get('/conversations/{chatbotLog}', [ChatbotLogController::class, 'show']);
        Route::post('/conversations/{chatbotLog}/reset', [ChatbotLogController::class, 'resetConversation']);
        Route::post('/conversations/{chatbotLog}/report', [ChatbotLogController::class, 'reportConversation']);
        Route::post('/conversations/{chatbotLog}/end', [ChatbotLogController::class, 'endConversation']);
        
        // Admin: monitor conversations
        Route::get('/admin/conversations', [ChatbotLogController::class, 'adminIndex'])->middleware('role:admin');
        Route::get('/admin/conversations/{chatbotLog}', [ChatbotLogController::class, 'adminShow'])->middleware('role:admin');
    });

    // ---- Rekomendasi ----
    Route::prefix('rekomendasi')->group(function () {
        Route::get('/', function (RecommendationService $service, Request $request) {
    ...
    // STAFF/ADMIN ROUTES
    // =====================================================================
    Route::middleware('role:admin,karyawan')->prefix('staff')->group(function () {
        // Dashboard & Reports
        Route::get('/dashboard', [AdminController::class, 'dashboard']);
        
        // Financial Reports
        Route::get('/laporan/keuangan', [LaporanController::class, 'financialSummary']);
        Route::get('/laporan/keuangan/chart', [LaporanController::class, 'revenueChart']);
        Route::get('/laporan/perilaku-pelanggan', [LaporanController::class, 'customerBehavior']);
        
        // Production Queue (KDS - Kitchen Display System)
        Route::get('/produksi/antrean', [LaporanController::class, 'productionQueue']);
    });
