    ...
    public function store(PesananRequest $request): JsonResponse
    {
        ...
        if (!$pelanggan) {
            return response()->json(
                [
                    "status" => "error",
                    "message" =>
                        "Profil pelanggan tidak ditemukan. Silakan lengkapi profil terlebih dahulu.",
                ],
                400,
            );
        }
        ...
        try {
            ...
            $this->notificationService->sendToStaff(
                "Pesanan Baru Masuk",
                "Pesanan baru #{$pesanan->id} dari {$user->name} dengan total Rp " .
    ...
    public function update(Request $request, Pesanan $pesanan): JsonResponse
    {
        ...
        if ($pelanggan && $pelanggan->user) {
            $statusLabels = [
                ...
                Pesanan::STATUS_PEMBAYARAN_DIBATALKAN =>
        ...
        return response()->json([
            ...
            "data" => $pesanan
                ->fresh()
                ->load(["pelanggan.user", "itemPesanans.produk"]),
        ]);
    }
