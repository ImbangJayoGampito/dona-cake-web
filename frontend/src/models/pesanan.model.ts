...
import { PesananStatus, type PesananStatus as PesananStatusType } from '@/types/enums'
...
export class Pesanan {
  ...
  constructor(data: Partial<Pesanan>) {
    ...
    this.tgl_pesanan = data.tgl_pesanan ? new Date(data.tgl_pesanan) : new Date()
    this.total_harga = data.total_harga ?? 0
    this.status_pesanan = data.status_pesanan ?? PesananStatus.MENUNGGU_PEMBAYARAN
  ...
  canTransitionTo(newStatus: PesananStatusType): boolean {
    const transitions: Record<PesananStatusType, PesananStatusType[]> = {
      ...
      [PesananStatus.PEMBAYARAN_DIBATALKAN]: [PesananStatus.MENUNGGU_PEMBAYARAN],
    }
