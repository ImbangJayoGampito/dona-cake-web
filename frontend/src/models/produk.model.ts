...
export class Produk {
  ...
  constructor(data: Partial<Produk> = {}) {
    ...
    if (data.ulasans) {
      this.ulasans = data.ulasans.map(u => u instanceof Ulasan ? u : new Ulasan(u))
    }
    if (data.gambars) {
      this.gambars = data.gambars.map(g => g instanceof Gambar ? g : new Gambar(g))
    }
    if (data.gambarUtama) {
      this.gambarUtama = data.gambarUtama instanceof Gambar ? data.gambarUtama : new Gambar(data.gambarUtama)
    }
  }
